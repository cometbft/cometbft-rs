...
pub mod block_results_service_server {
    ...
    #[async_trait]
    pub trait BlockResultsService: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct BlockResultsServiceServer<T> {
        inner: Arc<T>,
    ...
    impl<T> BlockResultsServiceServer<T> {
    ...
    impl<T> Clone for BlockResultsServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "cometbft.services.block_results.v1.BlockResultsService";
    impl<T> tonic::server::NamedService for BlockResultsServiceServer<T> {
