use cometbft_proto::v1::abci::v1beta1 as pb1;
...
impl TryFrom<pb::Request> for Request {
    ...
    fn try_from(request: pb::Request) -> Result<Self, Self::Error> {
        ...
        match request.value {
            ...
            Some(Value::Flush(pb1::RequestFlush {})) => Ok(Request::Flush),
            ...
            Some(Value::Commit(pb1::RequestCommit {})) => Ok(Request::Commit),
            Some(Value::ListSnapshots(pb1::RequestListSnapshots {})) => Ok(Request::ListSnapshots),
